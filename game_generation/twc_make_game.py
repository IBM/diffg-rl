...
class TWCGameMaker:
    ...
    def make_game(self):
        ...
        self.set_metadata(game, placed_objects, uuid)
        ...
        # self.reset()
        # return result, is_failed, game
        return game, uuid, is_failed

    def compile_game(self, game, uuid):
        self.config.game_options.path = pjoin(self.config.output_dir, uuid)
    ...
    def set_metadata(self, game, placed_objects, uuid):
    ...
    def place_furniture(self, distribute_evenly=True):
        ...
        nb_furniture = rng.randint(len(self.maker.rooms), len(self.data.locations) + 1)
        if distribute_evenly:
...
def run(config, entities=None):
    ...
    game_goals = []
    generated_entities = []
    while i < config.num_games:
        if config.verbose:
            print(f"Generating game {i + 1}\n")
        game, uuid, is_failed = twc_game_maker.make_game()
        if is_failed or list(game.metadata["goal_locations"].keys()) in game_goals or (entities is not None and len(set(game.metadata["goal_locations"].keys()) - entities) > 0):
            # print('Generation is failed')
            twc_game_maker.reset()
            set_difficulty_level(config)
            continue
        else:
            game_goals.append(list(game.metadata["goal_locations"].keys()))
            generated_entities.extend(list(game.metadata["goal_locations"].keys()))
        game_file = twc_game_maker.compile_game(game, uuid)
    ...
    return game_file_list, set(generated_entities)
...
def main():
    ...
    while i < config.num_games:
        if config.verbose:
            print(f"Generating game {i + 1}\n")
        game_file, is_failed = twc_game_maker.make_game()
        if is_failed:
            print('Generation is failed')
            set_difficulty_level(config)
            continue
