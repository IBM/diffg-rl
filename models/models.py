...
class RelationEncoder(nn.Module):
    def __init__(self, vocab, embed_dim, hidden_size, num_layers, dropout, bidirectional=True):
        ...
        self.rel_embed = PretrainedEmbeddings(vocab)
        self.rel_dim = self.rel_embed.dim
        self.rnn = nn.GRU(
            input_size=self.rel_dim,
...
class TokenEncoder(nn.Module):
    def __init__(self, token_vocab, embed_dim, hidden_size, dropout, bidirectional=True):
        super(TokenEncoder, self).__init__()
        self.token_embed = PretrainedEmbeddings(token_vocab)
        self.token_dim = self.token_embed.dim
        self.embed_dim = embed_dim
        self.dropout = dropout
        self.bidirectional = bidirectional
        self.hidden_size = hidden_size
    ...
    def forward(self, token_input):
        seqlen = token_input.size(0)
        token_repr = self.token_embed(token_input)
        token_repr = self.proj(token_repr)
        _, token_repr = self.rnn(token_repr)
        token_repr = token_repr.view(seqlen, self.embed_dim)
        token = F.dropout(token_repr, p=self.dropout, training=self.training)
...
class GraphEdgeAttentionLayer(nn.Module):

    def __init__(self, in_features, out_features, dropout, alpha, concat=True):
        super(GraphEdgeAttentionLayer, self).__init__()
        self.dropout = dropout
        self.in_features = in_features
        self.out_features = out_features
        self.alpha = alpha
        ...
        self.W_n = nn.Parameter(torch.zeros(size=(in_features, out_features)))
        nn.init.xavier_uniform_(self.W_n.data, gain=1.414)
        self.W_e = nn.Parameter(torch.zeros(size=(in_features, out_features//5)))
        nn.init.xavier_uniform_(self.W_e.data, gain=1.414)
        self.a_n = nn.Parameter(torch.zeros(size=(2 * out_features, 1)))
        nn.init.xavier_uniform_(self.a_n.data, gain=1.414)
        self.a_e = nn.Parameter(torch.zeros(size=(int(2.2 * out_features), 1)))
        nn.init.xavier_uniform_(self.a_e.data, gain=1.414)

        self.leakyrelu = nn.LeakyReLU(self.alpha)

    def forward(self, input, edge, adj):
        # 300を270と30に分割するとかもありかもしれない
        h = torch.matmul(input, self.W_n)
        p = torch.matmul(edge, self.W_e)
        N = h.size()[1]
        batch_size = h.size(0)
        # a_input_n = torch.cat([h.repeat(1, 1, N).view(batch_size, N * N, -1), h.repeat(1, N, 1)], dim=2).view(batch_size, N, -1, 2 * self.out_features)
        a_input_e = torch.cat([h.repeat(1, 1, N).view(batch_size, N * N, -1), h.repeat(1, N, 1), p.view(batch_size, N * N , -1)], dim=2).view(batch_size, N, -1, int(2.2 * self.out_features))
        # e_n = self.leakyrelu(torch.matmul(a_input_n, self.a_n).squeeze(3))
        e_e = self.leakyrelu(torch.matmul(a_input_e, self.a_e).squeeze(3))

        # zero_vec_n = -9e15 * torch.ones_like(e_n)
        zero_vec_e = -9e15 * torch.ones_like(e_e)
        # att_n = torch.where(adj > 0, e_n, zero_vec_n)
        att_e = torch.where(adj > 0, e_e, zero_vec_e)
        # attention = torch.cat([att_n, att_e], dim=2)
        # attention = torch.cat([e_n, e_e], dim=2)
        # attention = F.softmax(attention, dim=1)
        # attention_n = F.softmax(att_n, dim=2)
        attention_e = F.softmax(att_e, dim=2)
        ...
        # attention_n = attention[:, :, :N]
        # attention_e = attention[:, :, N:]
        # h_prime = torch.matmul(attention_n, h) + torch.matmul(attention_e, p).mean(2)
        h_prime = torch.matmul(attention_e, h)
...
class GEAT(nn.Module):
    def __init__(self, nfeat, nhid, dropout, alpha, nheads):
        ...
        self.attentions = [GraphEdgeAttentionLayer(nfeat, nhid, dropout=dropout, alpha=alpha, concat=True) for _ in
                           range(nheads)]
        for i, attention in enumerate(self.attentions):
            self.add_module('attention_{}'.format(i), attention)
...
class CQAttention(torch.nn.Module):
    ...
    def forward(self, C, Q, Cmask=None, Qmask=None):
